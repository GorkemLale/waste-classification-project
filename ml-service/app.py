...
WASTE_CLASSES = {
    0: {'name': 'paper', 'binType': 'Kağıt Atık', 'binColor': 'blue'},
    1: {'name': 'glass', 'binType': 'Cam Atık', 'binColor': 'green'},
    2: {'name': 'metal', 'binType': 'Metal Atık', 'binColor': 'gray'},
    3: {'name': 'organic', 'binType': 'Organik Atık', 'binColor': 'brown'},
    4: {'name': 'plastic', 'binType': 'Plastik Atık', 'binColor': 'yellow'}
}
...
def load_model():
    ...
    try:
        ...
        if os.path.exists(MODEL_PATH):
            ...
            print("⚠️  Custom model bulunamadı, YOLOv8n kullanılıyor (geçici)")
            model = YOLO('yolov8n.pt')  # Baseline model
...
def predict():
    ...
    try:
        ...
        # Model yüklü değilse mock data dön
        if model is None:
            print("⚠️ Model yüklü değil, mock data döndürülüyor")
            return jsonify({
                'success': True,
                'predictions': [
                    {
                ...
                'note': 'Mock data - model henüz yüklenmedi'
            })
